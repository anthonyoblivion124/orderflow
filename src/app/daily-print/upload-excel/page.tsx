  ...
  const onUpload = async () => {
    ...
    try {
      ...
      if (mode === "daily-print") {
        ...
        setMessage(
          payload.extractionOnly
            ? `Extraction complete: ${payload.extractedRows ?? 0} sales rows extracted from file.`
            : `Sales summary processed: matched ${payload.matchedGroups ?? 0}, updated ${payload.updatedRows ?? 0}, extracted ${payload.extractedRows ?? 0} rows.`
        );
      }
    } catch (error) {
      setIsError(true);
  ...
  return (
    <AuthGuard allowedRoles={["admin", "manager"]}>
      <MainAppLayout>
        <PageHeader
          title="Upload Excel"
          description="Upload DailyPrint or SalesByItemSummary CSV/Excel-export data as a separate workflow from Daily Print viewing."
          action={
            <div className="flex gap-2">
              <Button variant="outline" asChild>
                <Link href="/daily-print/upload-excel/extracted">View Extracted</Link>
              </Button>
            </div>
          }
        />
