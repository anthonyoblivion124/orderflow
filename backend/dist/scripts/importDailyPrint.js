...
function isMissingTableError(message) {
    ...
    return (lowered.includes("could not find the table") ||
...
async function main() {
    ...
    if (deleteError) {
        if (isMissingTableError(deleteError.message)) {
            throw new Error("Table public.daily_print_rows is missing. Run docs/supabase_schema.sql in Supabase SQL Editor, then retry import.");
        }
    ...
    console.log(`Imported ${mappedRows.length} rows into daily_print_rows for report_date=${reportDateIso}`);
}
main().catch((error) => {
