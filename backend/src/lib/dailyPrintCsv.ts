export type DailyPrintImportRow = {
  report_date: string;
  row_index: number;
  company_name: string | null;
  total: number | null;
  discount_ysb: number | null;
  discount_gift: number | null;
  discount_shop_discount: number | null;
  ...
  pos_amount_and_change: number | null;
  cash_received: number | null;
  banking_kpay_personal: number | null;
  banking_kpay_qr: number | null;
  banking_aya_pay: number | null;
  banking_kbz_bank: number | null;
  banking_aya_bank: number | null;
  banking_mab_bank: number | null;
  surplus_deficit: number | null;
};

export function parseDailyPrintCsv(data: string): string[][] {
  const rows: string[][] = [];
...
export function parseReportDateToIso(rawDate: string): string {
...
export function rowToImport(reportDate: string, rowIndex: number, row: string[]): DailyPrintImportRow {
  const values = [...row];
...
export function buildDailyPrintImportRows(csvText: string): {
  reportDateIso: string;
  rows: DailyPrintImportRow[];
} {
